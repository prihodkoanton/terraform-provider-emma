...
Kubernetes clusters can take significant time to provision. You can configure custom timeouts:

```terraform
resource "emma_kubernetes_cluster" "production_cluster" {
  ...
  # Configure timeouts for cluster operations
  timeouts {
    create = "60m"  # Cluster creation with multiple nodes
    update = "45m"  # Cluster updates and scaling
    delete = "30m"  # Cluster deletion
  }
}
```

The provider automatically waits for the cluster and all nodes to reach stable states during provisioning and updates.
...
Kubernetes clusters can be imported using their ID:

```shell
terraform import emma_kubernetes_cluster.example 12345
```
